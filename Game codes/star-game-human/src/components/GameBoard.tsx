...
import { faStar, faLock, faLockOpen, faRobot, faUser, faCat, faCrow, faDog, faFish } from '@fortawesome/free-solid-svg-icons';
...
export function GameBoard({ game, onLeaveGame }: GameBoardProps) {
  ...
  // Watch for all lock/unlock actions and show beam effects
  useEffect(() => {
    ...
    // Show beams for ALL lock/unlock actions (both human and bot)
    if ((latestAction.action === "lock" || latestAction.action === "unlock") &&
      latestAction.direction) {

      // Set the beam effect for any player, including target information
      setActiveBeam({
      ...
      // Set the targeted player for highlighting (if there is one)
      if (latestAction.targetPlayer !== undefined && latestAction.targetPlayer !== null) {
        setTargetedPlayer(latestAction.targetPlayer);
      } else {
        setTargetedPlayer(null);
      }

      // Clear the beam and target highlight after 1.5 seconds
      const timer = setTimeout(() => {
        setActiveBeam(null);
        setTargetedPlayer(null);
      }, 800);
  ...
  const getPlayerIcon = (playerIndex: number) => {
    ...
    // Choose specific animals for each human player
    let icon;
    switch (playerIndex) {
      case 0: icon = faCat; break;    // Team 1 human - Cat
      case 1: icon = faCrow; break;    // Team 1 bot
      case 2: icon = faDog; break;    // Team 2 human - Dog
      case 3: icon = faFish; break;  // Team 2 bot
    }
  ...
  const getPlayerSymbol = (playerIndex: number, isLocked: boolean = false) => {
    const { icon, color: defaultColor } = getPlayerIcon(playerIndex);
    ...
    // Icon classes - fix text color logic
    const iconColorClass = defaultColor;
    const iconClass = `text-2xl md:text-3xl ${iconColorClass}`;
